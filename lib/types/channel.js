...
export function createChannel(ctx, initialAgent, options) {
    ...
    const state = {
        ...
        removePending(id) {
            ...
            const updateInbox = agent.updateInbox;
            if (typeof updateInbox === 'function') {
                if (pendingItem.inboxItemId === undefined)
                    return false;
                if (updateInbox.call(agent, pendingItem.inboxItemId, { kind: 'remove' }) !== 'applied')
                    return false;
                    ...
                    try {
                        ...
                        if (first === undefined) {
                            ...
                            record.title = titled !== undefined
                                ? shortenTitle(String(titled.data.title))
                                : 'New session';
                            return;
                        }
...
${output}
    ...
    const bindAgent = () => {
        ...
        agentSubscriptions = [
            ...
            (() => {
                const normalizeInboxEvent = (...args) => {
                    const first = args[0];
                    let subject;
                    const ids = [];
                    if (first !== null &&
                        ...
                        if (Array.isArray(second)) {
                            for (const item of second) {
                                if (typeof item.message?.id === 'string')
                                    ids.push(item.message.id);
                            }
                        }
                        else if (second !== null && typeof second === 'object') {
                            if (typeof second.message?.id === 'string')
                                ids.push(second.message.id);
                        }
                    }
                    if (subject !== agent || ids.length === 0)
                        return;
                    const before = state.pending.length;
                ...
                const captureInboxItem = (...args) => {
                    ...
                    if (typeof itemId !== 'string' || typeof messageId !== 'string')
                        return;
                    let changed = false;
                ...
                const disposers = [];
                const onAgentEvent = (event, handler) => {
                    disposers.push(ctx.on(event, handler));
                };
