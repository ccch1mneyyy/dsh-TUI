...
export function createChannel(ctx, initialAgent, options) {
    ...
    const state = {
        ...
        async newSession() {
            ...
            const { route } = resolveModelRoute({ provider: options.configuredProvider, model: options.configuredModel }, readModelPref(), { provider: options.provider, model: options.model });
            try {
                handle = await agents.create({
                    sessionId,
                    meta: {
