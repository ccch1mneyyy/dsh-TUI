    ...
    const state = {
        ...
        async switchPreset(presetId) {
            ...
            try {
                ...
                state.notify(`Preset「${presetId}」不存在 · ${error instanceof Error ? error.message : String(error)}`, { color: 'error', timeoutMs: 8000 });
                return false;
            }
            ...
            if (!blank) {
                ...
                state.notify(`会话已开始，preset 已锁定（当前：${state.agentPreset ?? 'host'}）· 已保存为默认：${target.id}（/new 或下次启动生效）`, { color: 'warning', timeoutMs: 8000 });
                return true;
            }
            try {
                ...
                state.notify(`Preset 切换失败 · ${error instanceof Error ? error.message : String(error)}`, { color: 'error', timeoutMs: 8000 });
                return false;
            }
            state.emit();
            if (!writePresetPref(target.id)) {
