...
${output}
    ...
    const childStatsBySession = new Map();
    ...
    let pendingForegroundCard;
    const touchChildStats = (childId) => {
    ...
    const trackChildEvent = (childId, event) => {
        const stats = touchChildStats(childId);
        if (stats.firstEventAt === undefined)
            stats.firstEventAt = event.time;
        switch (event.type) {
            case 'tool/call':
                stats.toolUses += 1;
                break;
    ...
    const correlateDelegation = (card, name, resultText) => {
        if (card.tool === undefined)
            return;
    ...
    const syncJobIntoCard = (card, jobId) => {
        if (card.tool === undefined || jobs === undefined)
            return;
        let snapshot;
        try {
        ...
        card.tool.jobStatus = snapshot.status;
        if (snapshot.detail !== undefined)
            card.tool.jobDetail = snapshot.detail;
        const stats = card.tool.childStats ?? { toolUses: 0, totalTokens: 0 };
        if (stats.firstEventAt === undefined)
            stats.firstEventAt = snapshot.startedAt;
    ...
    const syncAllJobCards = () => {
        for (const card of state.rows) {
            if (card.kind === 'tool' && card.tool?.jobId !== undefined)
                syncJobIntoCard(card, card.tool.jobId);
        }
    };
    ...
    const bindAgent = () => {
        ...
        agentSubscriptions = [
            ...
            (() => {
                ...
                const disposers = [
                    ctx.on('subagent/start', startListener),
