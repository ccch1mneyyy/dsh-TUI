 ...
 */
export function resolveCtrlFlag(name, ctrl, superKey, isMac = isMacOS, extendedKeys = supportsExtendedKeys()) {
    if (!isMac || !extendedKeys)
        return ctrl;
    if (name !== undefined && CMD_RESERVED_KEYS.has(name))
        return ctrl;
    if (name === 'd')
        return ctrl || superKey;
    return !!superKey;
}
/**
 ...
 */
export function resolveCmdHomeEnd(name, superKey, isMac = isMacOS, extendedKeys = supportsExtendedKeys()) {
    if (!isMac || !extendedKeys || !superKey)
        return null;
    if (name === 'left')
        return 'home';
    if (name === 'right')
