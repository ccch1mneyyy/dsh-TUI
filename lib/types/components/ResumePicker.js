...
export function ResumePicker({ sessions, focusIndex, currentSessionId, mode, renameText, }) {
    ...
    return (_jsxs(Pane, { color: "permission", children: [_jsxs(Box, { flexDirection: "column", children: [_jsx(Box, { marginBottom: 1, children: _jsx(Text, { color: "remember", bold: true, children: "Resume" }) }), above > 0 && (_jsxs(Text, { dimColor: true, italic: true, children: ["\u2191 ", above, " more"] })), visible.map(session => (_jsx(ListItem, { isFocused: session.id === sessions[focusIndex]?.id, isSelected: session.id === currentSessionId, description: formatTimestamp(session.updatedAt), children: session.title || session.id }, session.id))), below > 0 && (_jsxs(Text, { dimColor: true, italic: true, children: ["\u2193 ", below, " more"] })), mode === 'confirm-delete' && focused !== undefined && (_jsx(Box, { marginTop: 1, children: _jsxs(Text, { color: "error", children: ["Delete \u201C", focused.title || focused.id, "\u201D? The session log is removed permanently."] }) })), mode === 'rename' && (_jsx(Box, { marginTop: 1, children: _jsx(SearchBox, { query: renameText, isFocused: true, isTerminalFocused: isTerminalFocused, placeholder: "New session name\u2026", prefix: "\u270E", borderless: true }) }))] }), _jsx(Text, { dimColor: true, italic: true, children: mode === 'list' ? (_jsxs(Byline, { children: [_jsx(KeyboardShortcutHint, { shortcut: "Enter", action: "confirm", bold: true }), _jsx(KeyboardShortcutHint, { shortcut: "Esc", action: "exit" }), _jsx(KeyboardShortcutHint, { shortcut: `${modLabel}d`, action: "delete" }), _jsx(KeyboardShortcutHint, { shortcut: `${modLabel}r`, action: "rename" })] })) : mode === 'confirm-delete' ? (_jsxs(Byline, { children: [_jsx(KeyboardShortcutHint, { shortcut: "Enter", action: "delete", bold: true }), _jsx(KeyboardShortcutHint, { shortcut: "Esc", action: "cancel" })] })) : (_jsxs(Byline, { children: [_jsx(KeyboardShortcutHint, { shortcut: "Enter", action: "save", bold: true }), _jsx(KeyboardShortcutHint, { shortcut: "Esc", action: "cancel" })] })) })] }));
}
