...
export function PromptInput({ channel, helpOpen, onToggleHelp, onRunCommand, selectionActive, fillText, onFillConsumed, onRewindRequest, }) {
    ...
    const flushPending = () => {
        ...
        try {
            ...
            setPendingQueue([]);
            channel.notify(`已发送 ${items.length} 条排队消息，当前回合结束后按序处理`, { timeoutMs: 4000 });
        }
        catch (error) {
