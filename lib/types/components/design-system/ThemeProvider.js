...
import { registerCustomThemeResolver, setActiveThemeName, setAutoThemeBase, getAutoThemeBase, AUTO_THEME_NAME, } from '../../theme.js';
 ...
 * standing choice: `auto` is valid everywhere a theme name is (CC_TUI_THEME,
...
export function ThemeProvider({ children, theme, }) {
    ...
    const [forcedValid] = useState(() => {
        ...
        if (isThemeAvailable(forced))
            return true;
        console.warn(`[dsh-tui] theme "${forced}" not found (built-ins: auto, light, dark, dark-ansi; user themes: ~/.dsh-cc/themes/*.json); falling back to auto-detection`);
        return false;
    });
    ...
    const value = React.useMemo(() => ({ theme: active ?? 'dark', autoBase, setTheme }), [active, autoBase, setTheme]);
    useEffect(() => {
        if (active !== null)
