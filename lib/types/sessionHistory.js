/**
 * Launcher contract for resuming a session: the TUI writes the chosen session
 * id to `~/.dsh-cc/resume.txt`, and a launcher feeds it back — the `dsh-cc`
 * wrapper (dsh-cc.cmd / dsh-cc) sets `DSH_CC_RESUME_SESSION`, and `-c` /
 * `--continue` / a bare `--resume` on `dsh --profile cc-tui` reads this file
 * directly (see src/args.ts parseCcTuiArgs). Session *records* live in
 * DSH's own persistence backend (jsonl or sqlite, per the composition) —
 * `/resume` lists those via `sessionPersistence.list()`, this file only
 * carries the id across processes. It also keeps a small `last-used.json`
 * of session-id → epoch-ms touches so `/resume` can sort
