 ...
 */
export function truncateContextText(text, max = CONTEXT_ENTRY_MAX_CHARS) {
    if (text.length <= max)
        return text;
 ...
 */
export function summarizeLoadedContext(context) {
    const parts = [];
    if (context.sections.length > 0)
        parts.push(`系统提示词 ${context.sections.length} 段`);
    if (context.files.length > 0)
        parts.push(`工作区指令 ×${context.files.length}`);
    if (context.contexts.length > 0)
        parts.push(`运行时上下文 ${context.contexts.length} 项`);
    if (context.skills.length > 0)
        parts.push(`技能 ${context.skills.length}`);
    if (context.tools.length > 0)
        parts.push(`工具 ${context.tools.length}`);
    return parts.join(' · ');
}
//# sourceMappingURL=loaded-context.js.map