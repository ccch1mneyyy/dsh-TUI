...
import { readModelPref, resolveModelRoute } from './modelPrefs.js';
...
export async function apply(ctx, config) {
    ...
    // Status-line route, resolved atomically (issue #67): the (provider,
    // model) pair is ONE decision — a cordis.yml override wins only when
    // both halves are set, the persisted `/model` choice wins as a whole
    // otherwise, and the harness default backs the rest. The same resolved
    // route feeds the fresh-session create inside resolveAgent, so the
    // display can never diverge from the actual request route.
    const modelPref = readModelPref();
    const { route } = resolveModelRoute({ provider: config.provider, model: config.model }, modelPref);
 ...
 *
 * Model route (issues #14/#30/#67): a create resolves the route atomically —
 * a cordis.yml `provider`+`model` pair wins only when both halves are set,
 * else the persisted `/model` choice wins as a whole, else the harness
 * default; a resume passes the config-only route through so the session's
 * own recorded route wins when cordis.yml sets nothing.
 */
async function resolveAgent(ctx, requestedSessionId, agentOptions, meta, configuredPreset) {
    ...
    // Fresh-session route, resolved atomically (issue #67): a cordis.yml
    // override wins only as a COMPLETE route (both halves set), the persisted
    // `/model` choice wins as a whole otherwise, and the harness default
    // backs the rest — the two halves can never come from different sources.
    const { route } = resolveModelRoute(agentOptions, readModelPref());
