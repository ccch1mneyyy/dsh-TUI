 ...
 */
export function completeCommands(input, roots = LOCAL_COMMANDS, children = () => []) {
    if (!input.startsWith('/') || /[\r\n]/u.test(input))
        return [];
    ...
    if (trailingSeparator && tokens.at(-1) === '')
        tokens.pop();
    const canonicalPath = [];
...
function resolveCompletionNode(candidates, token) {
...
function matchingCompletionToken(candidate, prefix) {
    if (candidate.name.toLowerCase().startsWith(prefix))
