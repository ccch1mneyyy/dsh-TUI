...
- 校验版本线:主 `0.1.2-alpha.3`,兼容 `0.1.1-rc.2` / `0.1.1-rc.1` / `0.1.0-rc.8` / `0.1.0-rc.7` / `0.1.0-rc.6`
...
- peer 范围:`^0.1.0-rc.6 || ^0.1.1-rc.1 || ^0.1.2-alpha.3`(契约外版本启动时打 drift 警告)
...
- **disabled overrides**:24 行。其中 23 行恒定禁用;`command-goal` 仅在
  ...
  因而 alpha 线与 web-app 对齐,rc.2 仍保留 host `/goal`;web-app 另有 `hmr`
...
- **inserts**:17 行(dsh-tui、working-activity、dsh-tui-auth、六个插件互通行,以及
  ...
  roots,alpha 线则省略 roots 并使用包内 `includeShippedRoot`
...
- 安装基线与验证线分离:dev 树由 `pnpm-workspace.yaml` 的 overrides 钉在
  rc.2,alpha 线不装进来,而是由 CI `alpha-compat` lane 对上游 tag 的源码做
...
- 上游删掉的包跟着删,不留半悬空的依赖;patch-surface 快照按 web-app 版本
  追加、保留历史。
- 业务 UI 代码原则上零修改;若 alpha 源码 tsc 报错,修复落在 `src/dsh-adapter/`
