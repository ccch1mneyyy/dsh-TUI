...
## 核心能力
  ...
  - **终端交互**：低资源占用，长会话稳定可靠；多种主题切换，样式美观，实时显示工作状态、TPS、缓存命中率等
    ...
    用户附图及助手/工具结果中的持久图片块会直接显示在会话记录中；Kitty graphics 可用时显示等比缩略图，否则保留同尺寸文字回退。全屏下点击输入框 `[Image #N]` 或 transcript 缩略图在对话区域居中打开大图预览，不遮挡输入栏（Esc/点击外部关闭）；Finder 复制的图片文件粘贴时直接入附件库为 `[Image #N]`。
