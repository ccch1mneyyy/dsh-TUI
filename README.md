...
## 核心能力
  ...
  - **终端原生交互**：流式 Markdown、结构化工具卡、命令与文件补全、`@` 文件引用
    （消息任意位置补全，文本附加内容，PNG/JPEG/WebP/GIF 作为持久图片块发送，
    支持 `@路径#L12-14` 行区间：只附加指定行，越界自动收敛或回退整文件并注明）、历史搜索、消息选择、
...
| 分组 | 命令 |
|---|---|
| 会话 | `/new` 新会话 · `/resume` 会话浏览器（搜索、预览、跨项目、折叠子 agent 运行） · `/rename` 重命名会话 · `/recap` 会话摘要（建议标题一键应用；`/settings` 可开「打开会话自动总结」，默认开，恢复会话时底部自动出现分隔线 + `回顾：` 摘要行，发送新消息后消失） · `/workspace resume|rename|open` 管理工作区 · `/clear` 清屏 · `/compact` 压缩 · `/export` 导出 Markdown · `/trace` 轨迹场景（亦可 `Ctrl+T`） · `/rewind` 回退选择器（同空输入双击 Esc 的时间回溯） · `/tree` 会话分叉树（跨 fork 会话拼接全部分支；悬停预览节点内容，点击弹出回退/从这分叉/切换分支菜单） · `/fork` 把当前会话复制为可恢复副本（原会话不受影响） · `/btw <问题>` 侧问（不打断主回合、不写历史） |
| 状态 | `/context` 已加载上下文明细 · `/status` 会话信息 · `/cost` token 用量 · `/doctor` 环境自检 · `/config` 配置来源 · `/init` 创建 AGENTS.md · `/settings` 设置面板（命名空间读取/编辑） |
| 模型 | `/model` 二级选择器（**最近使用**分组置顶（记录最近 10 个，`~/.dsh-tui/model-recents.json`）→ 顶层 provider 分组 → Enter 进入该组模型；单 provider 且无记录时直达列表；**切换 = fork 会话续聊，历史保留**） · `/effort` 推理强度（滑杆 / `status` / `<id>`） · `/preset` Agent 预设（**已开始会话不可切换**，blank-only） · `/thinking` 思考显示 · `/tokens` token 明细 · `/activity` 工作状态行动画（`frames <名>` / `status`） · `/theme` 主题选择器 · `/color`（无参调色板选择器；`<名>` 直接设置；`status`/`reset`）会话强调色（输入框边框 + 右上角会话名标签，按会话保存；标签默认关闭，`/settings` 可开） · `/lang` 中英界面切换（`/settings` 中亦可选择） |
| 账号/策略 | `/provider` 添加模型提供方（含内置 dsh-auth 的**订阅账号 OAuth 登录**分支：ChatGPT / Claude / Grok 免 API key，`/auth status\|login\|logout` 同源） · `/login` 凭证与账号状态 · `/logout` 登出说明 · `/permissions` 权限说明 · `/add-dir` 文件策略范围 · `/hooks` · `/mcp` |
