...
Windows 上启动时会把控制台输出代码页切到 UTF-8，且发生在首帧之前：dsh 的子进程层一律按 UTF-8 解码子进程输出，而原生工具（`powershell.exe`、`cmd.exe`、`git.exe`）按控制台代码页编码，中文系统下原本会整段变成替换字符；切换只做一次，会话内不再改代码页，避免控制台重发缓冲覆盖已画画面。
