...
assert.match(UPSTREAM_VALIDATED_LABEL, /0\.1\.2-alpha\.3/u)
...
mixedVersions['@deepseek-ai/dsh-agent'] = '0.1.2-alpha.3'
assert.deepEqual(installedUpstreamLines(mixedVersions), ['0.1.1-rc.2', '0.1.2-alpha.3'])
assert.deepEqual(upstreamDrift(mixedVersions), [])
assert.deepEqual(upstreamDriftSummary(mixedVersions), {
  kind: 'mixed',
  versions: ['0.1.1-rc.2', '0.1.2-alpha.3'],
})
