...
| Status | `/context` loaded-context details · `/status` session info · `/cost` session token usage and estimated spend · `/usage` cross-session usage and spend rollup · `/doctor` environment self-check · `/config` configuration sources · `/init` create AGENTS.md |
