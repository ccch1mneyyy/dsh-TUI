...
const dict = {
  ...
  'cost-note': { zh: '注：DSH 不提供费用计量，金额为本地价目表估算，实际以 provider 账单为准', en: 'Note: DSH meters no cost; the amount is a local price-list estimate — the provider bill is authoritative' },
  'cost-estimate': { zh: '估算花费 {{amount}} · {{model}} · {{turns}} 个回合', en: 'Estimated spend {{amount}} · {{model}} · {{turns}} turn(s)' },
  'cost-table': { zh: '价目表 {{source}}（{{asOf}}）· {{path}} 可覆盖，重启生效', en: 'Price list {{source}} ({{asOf}}) · override in {{path}}, applied on next launch' },
  'cost-unpriced': { zh: '{{turns}} 个回合未计价：价目表没有 {{model}} 的条目', en: '{{turns}} turn(s) unpriced: the list has no entry for {{model}}' },
  'usage-empty': { zh: '还没有用量记录 · 账本 {{path}}', en: 'No usage recorded yet · ledger {{path}}' },
  'usage-today': { zh: '今日      {{turns}} 回合 · {{tokens}} tokens · {{amount}}', en: 'Today       {{turns}} turn(s) · {{tokens}} tokens · {{amount}}' },
  'usage-week': { zh: '近 7 天   {{turns}} 回合 · {{tokens}} tokens · {{amount}}', en: 'Last 7d     {{turns}} turn(s) · {{tokens}} tokens · {{amount}}' },
  'usage-month': { zh: '近 30 天  {{turns}} 回合 · {{tokens}} tokens · {{amount}}', en: 'Last 30d    {{turns}} turn(s) · {{tokens}} tokens · {{amount}}' },
  'usage-all': { zh: '全部      {{turns}} 回合 · {{tokens}} tokens · {{amount}}', en: 'All time    {{turns}} turn(s) · {{tokens}} tokens · {{amount}}' },
  'usage-by-model': { zh: '按模型', en: 'By model' },
  'usage-model-row': { zh: '  {{model}} · {{turns}} 回合 · {{tokens}} tokens · {{amount}}', en: '  {{model}} · {{turns}} turn(s) · {{tokens}} tokens · {{amount}}' },
  'usage-unpriced': { zh: '（其中 {{turns}} 个回合未计价）', en: '({{turns}} of them unpriced)' },
  'usage-ledger-note': { zh: '账本 {{path}} · 只统计本机、自 {{since}} 起记录的回合', en: 'Ledger {{path}} · this machine only, recorded since {{since}}' },
  ...
  'cmd-desc-cost': { zh: '查看会话 token 用量与花费估算' },
  'cmd-desc-usage': { zh: '查看跨会话 token 用量与花费估算' },
