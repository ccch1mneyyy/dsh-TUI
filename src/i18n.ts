...
const dict = {
  ...
  'rename-current': { zh: '当前标题  {{title}}', en: 'Current title  {{title}}' },
  'rename-current-none': { zh: '（尚无标题）', en: '(no title yet)' },
  'rename-hint': { zh: '重命名    /rename <新标题>（固定后不再自动生成）', en: 'Rename      /rename <new title> (pins the title — no more automatic generation)' },
  'rename-renamed': { zh: '会话已重命名：{{title}}', en: 'Session renamed: {{title}}' },
  'rename-empty': { zh: '标题不能为空', en: 'Title must contain visible characters' },
  'rename-unavailable': { zh: '重命名不可用——当前组合未挂载 sessionTitle 服务', en: 'Rename unavailable — the sessionTitle service is not mounted in this composition' },
  'rename-failed': { zh: '重命名失败', en: 'Rename failed' },
