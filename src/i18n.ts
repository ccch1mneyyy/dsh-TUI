...
const dict = {
  ...
  'migrate-running': { zh: '迁移已启动（子进程运行中，界面不会卡顿）…', en: 'Migration started (running in a child process)…' },
  'migrate-done': { zh: '迁移完成，详见 /migrate 输出；用 /resume 按原目录浏览', en: 'Migration finished — see the /migrate output; browse by original directory via /resume' },
  'migrate-failed': { zh: '迁移进程退出码 {{code}}，详见输出', en: 'Migration exited with code {{code}} — see the output' },
  ...
  'migrate-picker-hint': { zh: 'Enter 导入 · Esc 关闭 · /migrate <agent> 可跳过选择', en: 'Enter import · Esc close · /migrate <agent> skips the picker' },
