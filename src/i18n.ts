...
const dict = {
  ...
  'fork-while-working': { zh: '回合运行中，无法分叉会话', en: 'Cannot fork the session while a turn is running' },
  'fork-unavailable': { zh: '会话分叉不可用——session 或 agent 服务未加载', en: 'Session fork unavailable — session or agent services not loaded' },
  'fork-source-failed': { zh: '无法读取当前会话的稳定分叉点 · {{err}}', en: 'Could not read a stable fork point · {{err}}' },
  'fork-create-failed': { zh: '创建分支会话失败 · {{err}}', en: 'Could not create the forked session · {{err}}' },
  'fork-attach-failed': { zh: '分支已创建，但工作区挂载失败 · {{err}}', en: 'Session forked, but workspace attachment failed · {{err}}' },
  'fork-done': { zh: '已创建并进入新分支', en: 'Forked into a new session' },
  ...
  'session-tree-title': { zh: '会话分支', en: 'Session branches' },
  'session-tree-count': { zh: '{{n}} 个节点', en: '{{n}} nodes' },
  'session-tree-loading': { zh: '正在读取分支关系…', en: 'Reading session lineage…' },
  'session-tree-empty': { zh: '当前会话尚未出现在持久化目录中', en: 'The current session is not in persistence yet' },
  'session-tree-current': { zh: '当前', en: 'current' },
  'session-tree-hint': { zh: '**Enter** 切换分支 · ↑/↓ 移动 · Esc 返回', en: '**Enter** switch branch · ↑/↓ move · Esc back' },
  'session-tree-already-current': { zh: '已经在这个分支中', en: 'Already on this branch' },
  'session-tree-resume-refused': { zh: '无法切换到该分支——原因已记录在对话里', en: 'Could not switch to that branch — the reason is in the conversation' },
  'session-tree-resume-failed': { zh: '切换分支失败 · {{err}}', en: 'Could not switch branches · {{err}}' },
  ...
  'cmd-desc-tree': { zh: '查看当前会话的分支树' },
  'cmd-desc-fork': { zh: '从当前历史创建新分支' },
