...
const dict = {
  ...
  'provider-opt-action-edit-desc': { zh: '修改密钥、端点或模型列表', en: 'Change the key, endpoint, or model list' },
  'provider-opt-action-delete': { zh: '删除已有 provider', en: 'Delete an existing provider' },
  'provider-opt-action-delete-desc': { zh: '移除配置与 API key', en: 'Remove the configuration and the API key' },
  'provider-q-edit': { zh: '选择要编辑的 provider', en: 'Choose a provider to edit' },
  'provider-q-delete': { zh: '选择要删除的 provider', en: 'Choose a provider to delete' },
  ...
  // /provider 编辑分支
  'provider-q-key-action': { zh: '{{route}} 的 API key 保留还是更换？', en: 'Keep or replace {{route}}’s API key?' },
  'provider-opt-key-keep': { zh: '保留现有密钥', en: 'Keep the existing key' },
  'provider-opt-key-keep-desc': { zh: '不改动 ~/.dsh/.credentials.yaml', en: 'Leave ~/.dsh/.credentials.yaml untouched' },
  'provider-opt-key-replace': { zh: '更换密钥', en: 'Replace the key' },
  'provider-opt-key-replace-desc': { zh: '覆盖写入新 key', en: 'Overwrite with a new key' },
  ...
  'provider-edit-note': { zh: '将覆盖 {{route}} 的现有配置', en: 'This overwrites the existing configuration of {{route}}' },
