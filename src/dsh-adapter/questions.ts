...
export class QuestionStore {
   ...
   * bare embeds). The pending ask is rejected with
   * {@link PLAN_REVIEW_EXITED} so the model-facing `exit_plan_mode` tool
   * reports "stop here" instead of treating the selection as keep-planning
   * feedback — and the installed handler aborts the calling agent's turn,
   * because a rejected tool result alone does not stop the agent loop from
   * continuing the unapproved plan.
   */
  exitPlanReview(): void {
    const pending = this.active
    const question = pending?.request.questions[pending.index]
    ...
    pending.reject(new UserQuestionError(
      'The user chose Exit planning: the plan was NOT approved and plan mode is off. Stop here and wait for the user’s next message.',
      PLAN_REVIEW_EXITED,
    ))
    this.startNext()
    this.emit()
