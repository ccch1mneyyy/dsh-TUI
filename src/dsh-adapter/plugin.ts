...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  // type (the legacy activity/status residue) into every reachable
  // KNOWN_SESSION_EVENT_TYPES copy BEFORE any strict read path can run
  // (--resume below, the /resume command, picker-backed loads). In-process
  // only: the shared session log is never rewritten. Idempotent and never
  // throws; unresolved trees degrade to the previous fail-closed behavior.
