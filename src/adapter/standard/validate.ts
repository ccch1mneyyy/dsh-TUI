...
export function createContractIndex(registry: ContractRegistry, permissions: PermissionRegistry): ContractIndex {
  ...
  const { protocols, manifests } = getAdmissionCatalog()
...
export function validatePlugin(index: ContractIndex, manifest: PluginManifest): void {
  ...
  // Derive the permission -> capability map from the machine-readable registry
  // instead of maintaining a second handwritten copy in the TUI product.
  const capabilityByPermission = new Map<string, { apiVersion: string; kind: string }>()
  for (const entry of registryEntries(index.registry)) {
    for (const permission of entry.permissions) {
      capabilityByPermission.set(permission, {
  ...
  for (const permission of manifest.permissions) {
    ...
    const capability = capabilityByPermission.get(permission.name)
