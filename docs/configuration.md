...
- **编辑已有 provider**：从已配置的路由中选择，进入编辑菜单——内置 provider
  可选 **编辑 API Key**、**编辑模型列表**、**删除该 provider**；自定义端点额外
  提供 **编辑 Base URL** 与 **编辑 wire protocol**。任一编辑项改完立即保存并
  退出，无需再确认；「编辑模型列表」会自动勾选当前已启用的模型。唯一例外是
  「删除该 provider」，需先确认，确认后移除 profile 与 API key（密钥来自环境
  变量时只删配置、不触碰环境变量）。
