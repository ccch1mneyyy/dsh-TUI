...
| `fullscreen` | `false` | `true` 使用 alternate screen、应用内滚动和鼠标选区；`false` 使用 inline 模式。会话内 `/tui fullscreen` / `/tui default`（及 `/settings`）写入 `~/.dsh/settings.yaml` 的 `dsh-tui.fullscreen`，用户层覆盖本组合默认 |
