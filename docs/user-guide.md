...
- **闲置动画**（`whaleIdle`，默认关）：开屏定格后鲸鱼继续摆鱼鳍、偶尔拍尾巴，agent 回合运行中持续游动，连续空闲 10 秒后睡着冒 Z；任何工作会立即唤醒。**点击鲸鱼冒爱心**始终可用（事件驱动，零空闲开销），与该设置无关。所有动作为**独立图层并行合成**（tail/fin/heart/sleep/blink 各自驱动）——爱心会叠加在摆尾或睡觉动作之上同时显示，而不是打断它们。
