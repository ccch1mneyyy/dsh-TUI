...
| 命令 | 参数 | 作用 |
|---|---|---|
| `/provider` | 无 | 交互式管理模型提供方向导（添加 / 编辑；编辑菜单含删除；持久化 profile，API key 非环境变量来源时才写入密钥库） |
...
- `/provider` 交互向导管理模型提供方（添加 / 编辑；编辑菜单可选 API Key、Base URL、wire protocol、模型列表或删除该 provider；非环境变量来源的密钥写入 `~/.dsh/.credentials.yaml` 0600，界面只显示 `••••••`；环境变量提供的密钥既不写入也不删除）。
