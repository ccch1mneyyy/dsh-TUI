...
| `/recap` | 无 | 最近活动摘要（一行）+ 建议标题；面板内 `a` 键或点击一键应用标题 |
...
| `/color` | `<名>` / `status` / `reset` | 会话强调色：输入框边框 + 会话名标签变色（`red/orange/yellow/green/blue/purple/pink/cyan`）。按会话经 `session/color` 事件保存，resume/rewind 后仍在；`reset` 恢复主题默认 |
