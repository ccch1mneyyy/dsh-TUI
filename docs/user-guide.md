...
| 命令 | 参数 | 作用 |
|---|---|---|
| `/provider` | 无 | 交互式管理模型提供方向导（添加 / 编辑；编辑菜单含删除，持久化 profile + key） |
...
- `/provider` 交互向导管理模型提供方（添加 / 编辑；编辑菜单可选 API Key、Base URL、wire protocol、模型列表或删除该 provider；密钥写入 `~/.dsh/.credentials.yaml` 0600，界面只显示 `••••••`）。
