...
| Status | `/context`, `/status`, `/cost`, `/balance` (official DeepSeek balance: summary row + hover details, click to refresh), `/config`, `/doctor`, `/init`, `/agents` (subagent dashboard; the status bar shows a `▸ N` chip while runs are live, hover lists them with elapsed times), `/jobs` (background jobs panel: status/elapsed/exit code, `k` kills), `/settings` |
