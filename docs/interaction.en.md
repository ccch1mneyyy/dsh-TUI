...
`intent: plan-review`). Only in Liangshen mode (whose plan mode injects the
plan prompt) does the first non-approve option show as `Exit planning`;
every other preset shows the asker's original label and treats it as
“continue planning”:
...
| `Enter` (`Exit planning` row) | Shown only in Liangshen mode: leave plan mode WITHOUT approving this plan; the model is told to stop and wait for the next message |
