...
- id: dsh-tui
  config:
    ...
    effort: max
    # Animation/theme development only; unsupported preview levels stay UI-only
...
| `previewEfforts` | unset | Extra UI-only `/effort` levels for animation previews. They change the TUI without persisting the choice or sending a level the current model did not advertise. Intended for theme/motion development, for example `[ultra]` |
