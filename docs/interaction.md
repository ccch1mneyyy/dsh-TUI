...
| 状态 | `/context`、`/status`、`/cost`、`/balance`（DeepSeek 官方余额：摘要行 + hover 明细，点击刷新）、`/config`、`/doctor`、`/init`、`/agents`（子代理面板；状态行在运行中子代理非零时显示 `▸ N`，hover 列出各次运行与时长）、`/jobs`（后台任务面板：状态/时长/退出码，`k` 停止）、`/settings` |
