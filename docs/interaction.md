...
四条命令和输入框行首的 `⌸` 按钮打开的是**同一个界面**：左边是工作区（durable
registry）栏，右边是所选工作区里的会话列表，每行带该会话的实时状态。它们原来
是三个各自演化的实现（会话浏览器 / 会话总览 / 工作区首页），同一个会话会被三个
界面各自列出、各自定义"打开"的含义，所以现在只剩一个界面、一套选择模型。

界面描述的是运行时的真实模型：**一个 TUI 进程同时托管多个会话**。被切走的会话
是**停放**，不是结束——正在跑的回合继续跑，切回来照旧。因此离开一个会话不需要
任何确认，也没有"停止"语义。

打开这个界面**不会丢掉没发出去的草稿**：输入框随界面卸载，草稿（文字、光标位置、
已暂存的图片引用，以及折叠块、全屏草稿编辑器的展开态和 vim 模式）由 Chat 保存，
`Esc` 回来时原样还给你。草稿跟着**会话**走——换了
会话，上一段对话的草稿不会跟过来；`/rewind` 回填的那条历史消息属于回退后的新会话，
因此照常留在输入框里。
...
工作区栏的菜单有四项：**编辑**（选中该工作区，右侧列出它的会话）、**新建会话**、
**重命名**、**从列表移除**（只删登记，目录与会话日志都保留）。工作区的加入方式
是"用这个目录启动 dsh-tui"（启动时自动登记），界面里没有"添加工作区"入口。

会话行的实时状态来自通道自己的 agent-view 投影（与状态栏的"等待输入"提示同源）：
运行中 / 等待输入 / 空闲 / 已完成 / 失败 / 已停止；当前终端所附着的那一行标
`当前`。点击会话行 = 进入该会话（同 `Enter`）；点击行内的 `★`/`☆` 只切换固定状态
（固定项持久化到 `~/.dsh-tui/session-pins.json`，**不会**进入会话）。

被**其他** TUI 终端占用的会话照常列出，但整行标红、行尾写 `占用 pid <pid>`，
并且**点不动**：两个进程驱动同一条 append-only 日志会交错写入、破坏转录。占用状态
按界面自有的 2 秒节拍重读账本，对方退出后会自动恢复可进入。本进程停放中的会话
不算占用。判定与文档见 [session-mount-runtime.md](session-mount-runtime.md)。

当工作区服务缺失（bare composition）或有会话所在目录**没有登记**时，栏里会出现
一个「未登记的工作区」分组，把这些会话照常列出并可恢复——"没有登记"不等于
"没有历史"。该分组只在本界面内存活，不会写回登记。

**旧界面相较之下的行为变更**（有意移除，不再回归）：会话级右键菜单（重命名/
删除单个会话）、`Ctrl+P` 固定快捷键、`Ctrl+S` 展开委托运行、`Ctrl+A`「仅本项目/
全部项目」、`Ctrl+B` 分支筛选、`Tab` 预览、`Ctrl+R` 重命名、`Ctrl+D` 删除、
`Ctrl+X` 清理空会话。会话的固定仍可通过行内 `★` 点击完成；后台会话的**派发与回复**、
`Space` 预览面板、`Shift+Enter` 派发并切换目前**没有**回到新界面，`/bg` 之后要进入
某个后台会话请在工作区列表里 `Enter` 它。
...
| 拖拽 | 选择文本，松开后立即复制并清除选区；`dsh-tui.scrollGutter: scrollbar` 时右缘滚动条是拖拽目标——未修饰左键按住拖动按轨道位置连续滚动（与点击轨道同一映射：拖到哪滚到哪），`Shift`/`Alt`/`Ctrl`+拖动仍是选字（拖拽协议只对无修饰左键开启） |
| 双击/三击 | 选择单词/整行并复制（`scrollGutter: scrollbar` 的轨道例外：轨道是拖拽目标，其上多击不再选行） |
...
以上鼠标行为只在 `fullscreen: true` 的 alternate screen 生效；inline 模式不开启
鼠标上报，滚动条拖动不适用（沿用终端原生 scrollback 与选区）。
...
| 分组 | 命令 |
| --- | --- |
| 会话 | `/new`、`/resume`、`/home`、`/agentview`（三者打开同一个会话管理界面）、`/bg`（别名 `/background`，会话转入后台并打开该界面）、`/rename`、`/recap`（最近活动摘要 + 建议标题一键应用；设置 `recapOnOpen` 开启时打开会话自动出分隔线 + `回顾：` 摘要行，发送新消息后消失，默认开）、`/workspace resume|rename|open`、`/clear`、`/compact`、`/export`、`/btw`、`/trace`（轨迹场景，亦可 `Ctrl+T`）、`/rewind`（时间回溯，同空输入双击 `Esc`） |
