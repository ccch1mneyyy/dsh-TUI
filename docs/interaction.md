...
（`intent: plan-review` 的专用决策布局）。仅梁神模式（进入 plan mode 即注入
plan 提示词）时，第一个非批准选项会显示为 `Exit planning`；其他 preset
显示提问方原始标签，按"继续规划"处理：
...
| `Enter`（`Exit planning` 行） | 仅梁神模式显示：不批准当前计划，直接退出 plan mode；模型收到停止并等待下一条消息的提示 |
