...
光标位于其起点时整块反显；点击它会把光标放到起点并打开预览。光标停在 token 的起点或末尾时预览自动打开，
离开时自动关闭；预览打开期间键盘仍归输入框，方向键可以在多张图片之间移动、预览随之切换。Esc 或点击卡片外只关闭当前
token 的预览，光标离开再回来会重新打开；点击 token 总是打开。手动输入或从历史恢复、没有附件能力的同形文本仍是普通文字。
全屏模式下点击输入框中的 `[Image #N]` 或 transcript 缩略图会在对话区域内打开同一个
居中大图预览，图片最多占对话区约七成宽、八成高，四周的对话、输入栏、状态行与置顶提示保持可见（Esc 或点击卡片外关闭；窄终端显示
纯元数据卡）。预览打开期间，卡片以外的对话区文字以终端的 faint 属性变暗，卡片本身、输入栏与状态行不变；关闭后恢复。卡片标题位于顶边框中央，格式为 `Image #N — 格式 · 宽×高 · 体积 · 文件名`；卡片至少与标题同宽，
小图不会挤掉文件名，标题超过对话区宽度时先在文件名中间省略。本次会话中从文件或剪贴板暂存的图片在卡片底行显示
来源路径（`路径: …`），过长时保留首尾、省略中间；从会话日志恢复的图片没有路径。失效的 `[Image #N]`（超过
128 个暂存或会话切换后）在点击、发送以及作为斜杠命令参数时都会给出明确警告。
发送后，用户图片会从持久化会话事件重新投影到 transcript；助手消息或工具结果
只要包含图片块，也走同一预览路径。全屏且 Kitty graphics 探测成功时显示有界、
...
| 单击输入框中的 `[Image #N]` / transcript 缩略图 | 打开居中大图预览（Kitty graphics 不可用时显示图片元数据）；预览打开时点击预览外部关闭 |
